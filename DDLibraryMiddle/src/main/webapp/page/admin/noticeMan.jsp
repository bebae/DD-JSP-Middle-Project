   ...
   const mypath = "<%=request.getContextPath()%>";
	$(function(){
	  $(".write").on("click", function(){
	    window.location.href = "<%=request.getContextPath()%>/notice/insertNotice.do";
	  });
	
      $(".delete-btn").on("click", function(){
          var noticeid = $(this).attr("id");
          var confirmation = confirm( noticeid + "번의 공지사항을 삭제하시겠습니까?" );
            
          if (confirmation) {
              var form = document.getElementById("noticeForm");
              $("#noticeid").val(noticeid);
              form.action = "<%= request.getContextPath()%>/notice/deleteNotice.do";
              form.submit();
          }
      });
...
<% 
	...
	List<NoticeVO> getAllNotice = (List<NoticeVO>) request.getAttribute("listNoticeAdmin"); 
    ...
    <div id="board-search">
      <div class="container">
        <div class="search-window">
          <form id="noticeSearchForm" action="<%=request.getContextPath() %>/notice/listNoticeAdmin.do">
                  <div class="search-wrap">
                    <input type="hidden" name="page" id="page" value="1">
                    <input id="searchWord" type="search" name="searchWord" placeholder="검색어를 입력해주세요." >
                    <button type="submit" id="searchbtn" class="btn btn-dark">검색</button>
                  </div>
                </form>
                
          <div id="write">
            <a class="write btn btn-dark">공지작성</a>
          </div>
        </div>
      </div>
    </div>
        ...
        <table class="board-table">
          ...
          %>
            <tr>
              ...
              <td><button id="<%=noticeVO.getNotice_id() %>" class="delete-btn">-</button></td>
            </tr>
          <%
          }
          %>
