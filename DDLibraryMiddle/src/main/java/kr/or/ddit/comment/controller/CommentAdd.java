...
@WebServlet("/comment/commentAdd.do")
public class CommentAdd extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		IPostService pservice = PostServiceImpl.getInstance();
		
		PostVO getPostDetail = pservice.getPostDetail(Integer.parseInt(postid));
		CommentVO getCommentDetail = service.getCommentDetail(Integer.parseInt(postid));
		
		request.setAttribute("commentDetail", getCommentDetail);
		request.setAttribute("postDetail", getPostDetail);
